...
plugin.init = async (params) => {
  ...
  // https://www.passkeys.com/guides
	...
	({ prefixes, pages, paths } = await plugins.hooks.fire('filter:passkeys.exemptions', { prefixes, pages, paths }));
...
plugin.addRoutes = async ({ router, middleware, helpers }) => {
	...
	routeHelpers.setupApiRoute(router, 'get', '/passkeys/register', middlewares, async (req, res) => {
	...
	routeHelpers.setupApiRoute(router, 'post', '/passkeys/register', middlewares, async (req, res) => {
	...
	routeHelpers.setupApiRoute(router, 'post', '/passkeys/verify', middlewares, async (req, res) => {
	...
	routeHelpers.setupApiRoute(router, 'delete', '/passkeys', middlewares, async (req, res) => {
		const { uid } = req;
...
plugin.getAuthnPublicKey = async (uid, id) => db.getObjectField(`passkeys:webauthn:${uid}`, id);

plugin.getAuthnCount = async id => db.sortedSetScore(`passkeys:webauthn:counters`, id);

plugin.updateAuthnCount = async (id, count) => db.sortedSetAdd(`passkeys:webauthn:counters`, count, id);
...
plugin.check = async ({ req, res }) => {
	...
	if (await plugin.hasKey(req.user.uid)) {
		if (!res.locals.isAPI) {
			...
			await controllerHelpers.formatApiResponse(401, res, new Error('[[passkeys:passkey-required]]'));
		}
...
plugin.adjustRelogin = async ({ req, res }) => {
	if (await plugin.hasKey(req.uid)) {
		...
		controllerHelpers.redirect(res, `/login/passkeys?next=${req.session.returnTo}`);
	}
};

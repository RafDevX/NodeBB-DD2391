...
async function getGroups(set) {
    ...
    groupNames = groupNames.filter(
        (groupName) => groupName && !groups.isPrivilegeGroup(groupName)
    );
...
Controllers.renderAuthnChallenge = async (req, res, next) => {
    ...
    if (
        ...
        ((req.query.next && !req.query.next.startsWith('/admin')) ||
            !req.session.tfaForce)
    ) {
        return res.redirect(
            nconf.get('relative_path') + (req.query.next || '/')
        );
    }
...
Controllers.renderSettings = async (req, res) => {
    ...
    if (
        ...
        (
            await groups.isMemberOfGroups(req.user.uid, tfaEnforcedGroups)
        ).includes(true)
    ) {
        forceTfa = true;
    }
